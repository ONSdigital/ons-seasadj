"""
A package for applying seasonal adjustment to time series data.
...
def _find_x12(x12path=None, prefer_x13=True):
    """
    Try to locate x12a or x13as on the PATH or in the X12PATH or X13PATH.
    ...
    if x12path is not None and x12path.endswith(_binary_names):
        ...
        if not Path.isdir(x12path):
            x12path = Path.parent(x12path)
...
def _check_x12(x12path=None):
    ...
    if not x12path:
        ...
        raise X13NotFoundError(error_message)
    return x12path


def _clean_order(order):
    """
    Convert spec arima order to a tuple of regular and seasonal orders.
...
def run_spec(x12path, specpath, outname=None, meta=False, datameta=False):
    if meta and datameta:
        error_message = "Cannot specify both meta and datameta."
...
def _make_var_names(exog):
    if hasattr(exog, "name"):
        ...
        error_message = "exog is not a Series or DataFrame or is unnamed."
        raise ValueError(error_message)
    try:
...
def _convert_out_to_series(x, dates, name):
    ...
    This function will convert x to a Dataframe where x is a string in the
    format given by x-13arima-seats output
    """
...
class SeriesSpec(Spec):
    """
    Create a template for spec files.
...
def pandas_to_series_spec(x):
    ...
    if hasattr(x, "columns"):  # convert to series
        if len(x.columns) > 1:
            error_message = "Does not handle DataFrame with more than one column"
            raise ValueError(error_message)
        x = x[x.columns[0]]

    data = "({})".format("\n".join(map(str, x.to_numpy.tolist())))

    # get periodicity
    # get start / first data
    # give it a title
    try:
        period = FreqToPeriod[x.index.freqstr]
    except (AttributeError, ValueError):
        from pandas.tseries.api import infer_freq

        period = FreqToPeriod[infer_freq(x.index)]
...
def x13_arima_analysis(
    ...
    try:
        ...
        try:  # sometimes this gives a permission denied error?
            ...
            Path.unlink(ftempin.name)
            Path.unlink(ftempout.name)
        except OSError:
